...
- [ ] Iter 88：把 panel "prompt: N 条 hint" badge 升级为可点击展开——hover 已有 tooltip，但点击后 inline 展示"每条规则前几个字"列表（从 backend 同时返 `Vec<{label, summary}>`）。让用户不用 hover 就能看到当前 prompt 状态详情。
