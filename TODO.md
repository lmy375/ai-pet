...
- [x] Iter R82：R81 deadline-shrunk cadence 在 panel chip + decision_log 可见化（2026-05-04 完成 — R81 透明度审计）。
  R81 chip 自身视觉无变化，user 必须 hover 才能看到加速。R82 chip text 加 ⚡ 红色角标（factor<1.0 时）+
  hover summary "cadence ×N 加速"高层概括（1/factor 倒推自适配）。后端加 pure helper
  annotate_skip_with_deadline_factor：cooldown skip + factor<1.0 时附 [deadline-shrunk × 0.5] 后缀
  写入 decision_log。4 新单测；619 tests pass）
- [x] Iter R83：extreme-absence-reunion (≥24h) 升级 register（2026-05-04 完成 — Cν 4h tier 升级）。
  Cν 在 4h 用 casual welcome 重逢感；user 消失 ≥24h 时 register 应 shift 到 gentle check-in concern。
  新 EXTREME_ABSENCE_MINUTES=1440 常量；active_composite_rule_labels 在 ≥24h 时 push extreme-absence-reunion
  并抑制 long-absence-reunion（mutually exclusive）；prompt_assembler 加专门 directive 教 LLM "好久没见到你了，
  还好吗" 而非 "刚回来呀"；4 新单测覆盖 boundary + mutual exclusion + gating reuse；623 tests pass）
