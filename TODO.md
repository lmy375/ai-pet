...
- [ ] Iter 83：当 env_tool 环境感知率持续偏低（如 spoke_total > 10 且 spoke_with_any/spoke_total < 30%）时，在 proactive_rules 里临时加一条"上几次开口你没看环境就说话，本次试试 get_active_window 看看用户在做啥"的纠偏 hint。让数据反过来强化 prompt。
