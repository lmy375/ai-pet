...
- [ ] Iter 93：第二条复合规则候选——"long-idle-no-restraint"（since_last_proactive >= 60min && today_count < threshold && !pre_quiet）：很久没说话 + 今天没聊太多 + 不是安静时段，鼓励主动找话题。需要把 since_last_proactive 引入 PromptInputs（当前只有 cadence_hint 字符串，没有数字字段供规则做精确判断）。
