...
- [ ] Iter 98：把 PROMPT_RULE_DESCRIPTIONS 和 6 个 stat interface 从 PanelDebug.tsx 抽到 `src/components/panel/types.ts`（或 `panelTypes.ts`），让 PanelDebug 只负责 state + layout。同步更新 cargo 对齐测试 parse_prompt_rule_dict_keys 路径。
