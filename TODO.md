...
- [ ] Iter 53：让 `proactive_rules()` 接 `&PromptInputs` 参数，按当前上下文动态选择规则——比如 wake_hint 非空时插一条"用户刚回来，问候要简短"；mood 没记录时插一条"先 create current_mood 再说话"。当前 rules 是固定 6 条，没用上 builder 模式开放的灵活性。
