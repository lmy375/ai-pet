...
- [ ] Iter 82：在 ProcessCounters 加 EnvToolCounters（per-tool atomic：active_window / weather / upcoming_events / memory_search）每次 Spoke 时基于 outcome.tools bump，panel 加"环境感知率"chip 显示 spoke 里有多少比例用了至少一个 env 工具。Iter 81 的 tool tags 在 decision log 是离散信号，需要聚合数据观察 prompt 是否在驱动 LLM 用工具。
