...
- [ ] Iter R6：feedback_history 在 panel timeline 可见。
  - AI prompt：R1 写入 feedback_history.log（replied / ignored）但 panel 看不到。加 Tauri 命令
    `get_recent_feedback`（已有内部 helper），DebugSnapshot 加字段，PanelDebug 折叠卡显示
    最近 N 次 replied/ignored 时间线。是 R4 (tool call history) 的同模式。

- [ ] Iter R7：feedback signal 驱动 cooldown 调整。
  - AI prompt：用过去 24h 的 ignored/total 比率自适应调 cooldown：高 ignore（>60%）→ cooldown
    2x；低 ignore（<20%）→ cooldown × 0.7。pure helper `adapted_cooldown(base, ignore_ratio)`
    + 嵌入 gate 检查。让"被忽略多 → 自然安静下来" 形成闭环。需要先做完 R6 让 ratio 可观察。
