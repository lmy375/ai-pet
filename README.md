...
- **任务详情 bare URL 渲染为 📎 chip**：detail.md 里贴的 bare https/http URL 自动升级为「📎 hostname」chip 卡片（点击调 `plugin-opener` 打开默认浏览器，hover 显完整 URL）。negative lookbehind 排除 markdown 链接 `[text](url)` 里的 url —— 那种有显式锚文本路径仍走 parseMarkdown 标准超链接渲染。trailing 标点（句号 / 逗号 / 引号）剥到尾巴让"看这里 https://a.com。"不把"。"吃进 URL。让 detail.md 里的引用链接以附件形态独立呈现，与正文文字视觉分层。
...
- `⌘1` – `⌘5`（含 Ctrl 等价）跳到对应 tab（设置 / 聊天 / 任务 / 记忆 / 人格），Chrome / Slack / Linear 风的肌肉记忆；输入框聚焦时自动让出键位不打扰打字。调试窗口同款 `⌘1` – `⌘4` 跳到（应用 / 日志 / LLM 日志 / 统计）。
