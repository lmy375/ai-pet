...
- **复用 shared urgent_count 跨 chip + cooldown breakdown + gate 三个 caller**：build_tone_snapshot 之前在 ToneSnapshot 字面量内 inline 算 urgent_count（R78 surface chip）。R81 把它 lift 到 outer scope，cooldown_breakdown 和 chip 共享同一 count。同时 gate.rs evaluate_loop_tick 独立 fetch 一次（async path 不能借用 build_tone_snapshot 的 sync 结果）。**panel = build_tone_snapshot；gate = evaluate_loop_tick**——两个独立读取点都用同一个 count_urgent_butler_deadlines + parse_butler_deadline_prefix，single source of truth at helper 层而非 cache 层。
- **chip color 不变，只在 hover 拼额外段**：deadline 紧迫时不去染 chip 颜色——已有 R28 codified 的 feedback band 颜色 (high_negative amber / low_negative green / mid cyan) 占用了 chip color 通道。R81 在 hover text 里多拼一段 "× 0.5 (deadline 紧迫 N)" 而非 override 颜色。**避免颜色 channel 多个信号 fight**——一个 chip 一个 color reason；多信号通过 hover 文本 stack。
- **build_cooldown_breakdown 签名 +1 arg 的 ergonomic decision**：考虑过让 build_cooldown_breakdown 内部自己 fetch urgent_count（封装），但 rejected——这函数原本签名只接 recent_fb，要它内部去 IO（memory_list butler_tasks）会让 pure-data → impure 升级。改为传 urgent_count 进来，让 caller (build_tone_snapshot + 未来其他 caller) 决定 IO 时机。**function purity 边界保留**——breakdown 仍是接数据进出数据，IO 留给 caller。
- **gate.rs 的 deadline IO 是新增 IO，可接受**：evaluate_loop_tick 之前不读 butler_tasks，R81 加了一次 memory_list IO（每 60s 一次 tick）。同 R62 deep-focus gate 加 osascript IO 的先例——gate 升级时新 IO 可接受，理由：cadence 决定 fidelity，pet "real-time partner" feel 比每 tick 多 1 file read 的成本重要。
- **没给 build_cooldown_breakdown 写专门 unit test**：build_cooldown_breakdown 读 get_settings()（全局可变 state），写 unit test 需 TEST_LOCK + 临时 override settings。pure helper deadline_urgency_factor 已 3 测覆盖（zero / single / many），breakdown 内的应用是 一行 `* deadline_factor`，integration risk 低。**测试投入回报比**——pure 层完整覆盖，integration 层薄包装可手验。
- **下一 iter 候选**：(1) panel "deadline 触发 cooldown 缩短" 视觉 chip（让 chip 颜色 / 形态变化反映 cadence shift，目前仅 hover 文本可见）；(2) gate skip 时如果发现 urgent deadline，把 skip reason 升级到 "Skip - cooldown × 0.5"，让 panel decision_log timeline 看到 R81 在介入；(3) 把 deadline_factor 也注入 telegram bot 的 cadence；(4) cluster 真正切换方向——idle wake-up auto-mute 推断 / morning ritual 等新概念。
