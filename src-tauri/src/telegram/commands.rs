...
/// `/cat_decay_7d` 命令回复文案。pure：caller (bot.rs) 已计算「max
/// items.updated_at < now-7d」cat list + 每条最后 update 距 now 的天
/// 数（rounded floor）+ 按 days desc 排好（最老 stale 在上）。format
/// 拼文案 + 空兜底。row：(cat_key, label, days_since_update)。
pub fn format_cat_decay_7d_reply(
    rows: &[(String, String, i64)],
) -> String {
    if rows.is_empty() {
        return "🍂 所有 cat 7d 内都有 update 活动 — 健康状态。\n试 /cat_growth_7d 看正向（哪类在长）。".to_string();
    }
    let mut out = format!(
        "🍂 近 7 天 0 update 活动的 cat（共 {} 条 stale）：",
        rows.len(),
    );
