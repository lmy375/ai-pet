...
/// 在 description 末尾追加 `[done]` + 可选 `[result: ...]`，幂等：已含 done
/// 标记就原样返回。result 为 None / 空串 / 仅空白 → 仅 `[done]` 不附 result；
/// 非空 trim 后追加 `[result: <trimmed>]`。面板"标 done" / 命令行快捷键路径
/// 都调本 fn；不需要 result 时传 None。
